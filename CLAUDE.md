...
- Branch naming: `claude/<descriptive-slug>` (e.g. `claude/design-polish`, `claude/add-game-page`, `claude/fix-swipe-double-bounce`).
...
- **Emojis are welcome here** even though the global default is no-emoji. The /game page in particular leans heavily on them (👹 😉 🦄 🤬 💀); match that flavor when extending it.
...
- Don't add documentation files (other than this one) unless asked. Yaakov communicates intent through code, commits, and PRs.
